...
void applyGlidepath(FlightData& data, const MapData& map) {
  ...
  // Only let the glidepath own the vertical-deviation scale once it supersedes
  // the VNAV descent (aircraft within the capture window). Before that the VNAV
  // "V" deviation stays shown while VPTH flies down to the approach.
  if (gp.valid && glidepathSupersedesVnav(gp)) {
